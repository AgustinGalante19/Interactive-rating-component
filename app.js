...
const puntos = document.querySelectorAll(".circle")
const submit_btn = document.getElementById("submit")
const thank_card = document.querySelector(".thanks")
const result = document.querySelector(".puntuacion")

for (let i = 0; i < puntos.length; i++) {
    const punto = puntos[i]
    punto.addEventListener("click", () => {
        let punto = puntos[i].innerHTML

        for (let i = 0; i < puntos.length; i++) {
            puntos[i].classList.contains("selected")
            puntos[i].classList.remove("selected")
        }

        puntos[i].classList.add("selected")

        submit_btn.addEventListener("click", () => {
            card.style.display = "none"
            thank_card.style.display = "block"
            console.log(punto.trim())
            result.innerHTML = `<p style="padding: 0; margin:0; color: #fb7413;">You have selected ${punto} of 5</p>`
        })
    })
}
